...
## Relational Visual Similarity (relsim)

 CLIP 으로 모델 학습해서 유사성을 보면 attribute-similarity 는 잘 잡는데 logical-similarity는 잘 못 잡음.

 예를 들어 콜라와 사이다의 유사도는 높게 나오지만 콜라 1개와 생수 1개의 유사도는 낮게 나오는 편임. 일반적으로는 이 부분은 이미지 벡터가 존재하는 manifold의 한계라고 인지하고 있지만 저자들은 학습을 통해 해결가능하다고 주장.

 논리적 유사성을 CLIP구조에 학습시킴. 논리적 유사성을 가지는 데이터셑을 어떻게 만드냐고? 

 - 수동으로 논리적 유사성을 가지는 이미지쌍을 가져옴.
 - 논리적 유사성을 가지는 이미지 셑의 설명 문구를 생성. 단 여기서 이미지의 속성 묘사가 아니라 논리적 관꼐 묘사에 집중함. 
 - 그리고 논리적 관계 유사성을 기반으로 이미지 - 이미지, 이미지-텍스트간 유사도도 계산해서 이를 바탕으로 CLIP 학습 시킴.